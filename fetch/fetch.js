...
router.post("/student", (req, res) => {
    ...
    router.get("/", (req, res) => {
        ...
        mcl.connect(url, (err, conn) => {
            ...
            if (err)
                ...
                let db = conn.db('CRS')
                db.collection('collegeori').find().toArray((err, array) => {
                    if (err)
                        console.log('Error while fetching data')
                    else {
                        console.log('Data sent')
    ...
    router.post("/collegeori", (req, res) => {
        //connect to mongodb
        ...
        mcl.connect(url, (err, conn) => {
            ...
            if (err)
                ...
                db.collection('collegeori').find({ Region: clgregion }).toArray((err, array) => {
                    if (err)
                        console.log('Error while fetching data')
                    else {
                        console.log('Data sent')

                        const filterResult = array.filter(collegeori => collegeori[clgbranch] !== undefined);
                        const finalResult = array.filter(collegeori => collegeori[clgbranch] >= minMerit && collegeori[clgbranch] <= maxMerit);
